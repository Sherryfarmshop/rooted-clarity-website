          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-white rounded-2xl p-7 shadow-sm border border-[#e5dfd5]">
                ...
                <p className="text-gray-400 text-xs mt-3 italic">Don&apos;t have a digital copy? Describe your key values in the notes section below.</p>
              </div>

              {/* Symptoms by system */}
              <div className="bg-white rounded-2xl p-7 shadow-sm border border-[#e5dfd5]">
                <h2 className="font-serif text-xl font-bold text-[#1B3A2A] mb-2">What Are You Experiencing?</h2>
                <p className="text-gray-400 text-sm mb-6">Select everything that applies across any body system.</p>

                <div className="space-y-6">
                  {symptomGroups.map((group) => (
                    <div key={group.label}>
                      <p className="text-[#1B3A2A] text-xs font-bold uppercase tracking-widest mb-3 pb-1 border-b border-[#e5dfd5]">
                        {group.label}
                      </p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {group.symptoms.map((s) => (
                          <label key={s} className="flex items-start gap-2.5 cursor-pointer group">
                            <input type="checkbox" name="symptoms" value={s} className="mt-0.5 w-4 h-4 rounded border-[#e0d8cc] accent-[#1B3A2A]" />
              ...
              <div className="text-center">
                <button type="submit" disabled={loading} className="inline-block bg-[#C9A84C] text-[#1a1a1a] font-bold px-12 py-5 rounded-xl text-base uppercase tracking-widest shadow-xl hover:bg-[#b0873a] hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
